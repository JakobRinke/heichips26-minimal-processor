...
module heichips26_minimal_multicore_processor (
    ...
    logic [ 1:0] opcode; // 0 = wait, 1 = receive adress
    ...
    // Byte-Mask
    assign sram_bm = 32'hFFFF_FFFF;

    // uio_in 
    assign uio_in = sram_dout[7:0];

    //Memory logic interface connect to eFPGA SRAM
    always_ff @(posedge clk1 or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (uio_oe == 8'hFF && opcode == 0) begin
                // first byte = adress
                sram_addr <= {2'b00, uio_out};
                opcode    <= 2'd1;

            end else if (uio_oe == 8'hFF && opcode == 1) begin
                // second byte write
                sram_din <= {24'b0, uio_out};
                sram_wen <= 1'b1;
                sram_men <= 1'b1;
                opcode   <= 2'd0;
            end

            if (uio_oe == 8'h00 && opcode == 1) begin
                // receive again, load available
                sram_ren <= 1'b1;
                sram_men <= 1'b1;
                opcode   <= 2'd0;
            end
